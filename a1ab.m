...
dt = T / M;                    % Time step
A = 1.0;                       % Fixed Amplitude of the time-dependent potential

% Set of omega values from 0.01 to 0.1 with 5 elements
omega_values = linspace(0.01, 1, 20);
...
[X_grid, omega_grid] = meshgrid(X, omega_values);
probability_density = zeros(length(omega_values), length(X));
...
% Plotting the 3D plot where omega is an independent axis
figure;
surf(X_grid, omega_grid, probability_density);
xlabel('Position X')
