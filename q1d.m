...
% Initialize matrix to store maximum probability density and probability distributions
max_probability_density = zeros(length(A_values), length(omega_values));
probability_distributions = zeros(length(A_values), length(omega_values), length(X));
...
for idx_a = 1:length(A_values)
    ...
    for idx_omega = 1:length(omega_values)
        ...
        probability_distributions(idx_a, idx_omega, :) = abs(psii).^2;
    end
end

% Plotting the 3D plot where X, A, omega, and probability density are axes
figure;
[X_grid, A_grid, omega_grid] = ndgrid(X, A_values, omega_values);
probability_flat = reshape(probability_distributions, [], length(X));
scatter3(X_grid(:), A_grid(:), omega_grid(:), 10, probability_flat(:), 'filled', 'MarkerEdgeColor', 'none');
xlabel('Position X')
ylabel('Amplitude A')
zlabel('Frequency \omega')
title('3D Plot of Probability Density vs X, A, and \omega')
colorbar
